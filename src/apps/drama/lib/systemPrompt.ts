...
export function buildSystemPrompt(
	...
	return [
		...
		`— 树节点 —`,
		`- spellpaw_add_node / spellpaw_update_node / spellpaw_delete_node / spellpaw_move_node — 维护幕/场景/镜头骨干`,
		`- spellpaw_get_tree / spellpaw_get_subtree — 查结构，get_subtree 可拿场景级元数据（shotType / cameraMovement / dialogue）`,
		`— 画布内容生成 —`,
		`- spellpaw_generate_asset / spellpaw_generate_variants / spellpaw_edit_asset`,
		`- spellpaw_apply_style / spellpaw_batch_apply_style`,
		`- spellpaw_generate_storyboard — 从 tree 场景生成 AI 参考图卡片`,
		`— 模板与分析 —`,
		`- spellpaw_apply_template (templateId) — 套用叙事模板创建结构`,
		`- spellpaw_match_template — 智能匹配模板`,
		`- spellpaw_kickstart_project — 一键生成项目结构（模板+场景卡）`,
