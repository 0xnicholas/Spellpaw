...
export function OutputCardNode({ data, id, selected }: NodeProps<Node<CanvasNodeData>>) {
  ...
  return (
    <div
      className={`w-[240px] rounded-[var(--radius-base)] border bg-[var(--color-bg-secondary)] shadow-sm transition-shadow overflow-hidden ${
    ...
    >
      ...
      <div className="p-3">
        {isEditing ? (
          ...
          <h4 className="text-sm font-medium text-[var(--color-text-primary)] cursor-text m-0"
        ...
        {summary && (
          <p className="mt-2 text-[11px] text-[var(--color-text-secondary)] leading-relaxed m-0">{summary}</p>
        )}
      </div>
