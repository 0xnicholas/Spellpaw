...
│   ├── components/ui/        # 通用 UI（Button, IconButton, Modal, TabBar, FormField, Lightbox...）
│   ├── stores/
...
✅ 阶段 3: Pandaria 对接（session 配置 + SSE 消费）
  → src/apps/drama/hooks/usePandariaSSE.ts + src/apps/drama/lib/pandaria.ts
...
- Phase 3 新增：`src/shared/lib/idbStorage.ts`、`src/apps/drama/lib/syncEngine.ts`、`src/apps/drama/lib/migrateToIDB.ts`、`src/apps/drama/lib/treeDiff.ts`、`src/apps/drama/lib/projectAnalysis.ts`、`src/apps/drama/lib/templateExportImport.ts`、`src/apps/drama/stores/customTemplateStore.ts`
...
*最后更新：2026-05-31（多 App 架构：新增 Portal Landing 首页 + Drama Studio 封装）*
