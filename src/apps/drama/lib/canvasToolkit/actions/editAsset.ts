...
export async function editAsset(params: EditAssetParams): Promise<ToolkitResult> {
  ...
  const sourceCard = useCanvasStore.getState().getCurrentNodes().find((n) => n.id === params.cardId);
  if (!sourceCard) {
  ...
  const preferredCapabilities: Capability[] = ['inpaint', 'image2image', 'text2image'];
  ...
  if (task.status === 'failed') {
    return { success: false, message: task.error ?? '图像编辑失败', retryable: true };
  }

  const title = linkedNode ? `${linkedNode.title}（编辑版）` : `${sourceCard.data.title}（编辑版）`;
