...
- 不改动现有 Agent 逻辑（chatStore、useCopilotSSE、toolRouter 等）
...
- **ChatPanel**：装配层，从 `chatStore` 取数据，注入 `useCopilotSSE` hook，传给 CopilotChat
...
- `useCopilotSSE.ts` / `useMockSSE.ts` / `useToolBridge.ts` / `useTaskSSE.ts`
- `toolRouter.ts`
- `lib/copilot.ts`
