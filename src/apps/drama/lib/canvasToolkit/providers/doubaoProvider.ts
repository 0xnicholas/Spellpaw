...
export function createDoubaoProvider(): GenerationProvider {
  ...
  return {
    ...
    capabilities: ['text2image', 'image2image', 'styleTransfer', 'text2video', 'image2video'],
    requiredConfigKeys: ['doubaoApiKey'],

    isConfigured() {
      if (typeof config.apiKey === 'string' && config.apiKey.length > 0) return true;
      const settings = readSettings();
      return typeof settings.doubaoApiKey === 'string' && settings.doubaoApiKey.length > 0;
    },
    ...
    configure(next) {
      const apiKey =
        typeof next.apiKey === 'string' && next.apiKey.length > 0
          ? next.apiKey
          : typeof next.doubaoApiKey === 'string' && next.doubaoApiKey.length > 0
            ? next.doubaoApiKey
            : undefined;
    ...
    async poll(taskId: string): Promise<GenerationTask> {
      ...
      try {
        ...
        if (!res.ok) {
          return { taskId, status: 'failed', error: extractError(data) ?? `HTTP ${res.status}` };
        }
        ...
        if (status === 'failed' || status === 'error') {
          return { taskId, status: 'failed', error: extractError(data) ?? '视频生成失败' };
        }
...
async function submitImage(
  ...
  const images: string[] = [];
  if (input.referenceUrl) images.push(input.referenceUrl);
  const styleUrl = input.options?.styleReferenceUrl as string | undefined;
  if (styleUrl) images.push(styleUrl);
  ...
  try {
    ...
    if (!url) {
      return { taskId: '', status: 'failed', error: 'No image URL in Doubao response' };
    }
    return { taskId: `doubao_img_${Date.now()}`, status: 'done', resultUrl: url };
  } catch (err) {
    return { taskId: '', status: 'failed', error: (err as Error).message };
  }
}
...
async function submitVideo(
  ...
  try {
    ...
    if (!taskId) {
      return { taskId: '', status: 'failed', error: 'No task id in Doubao video response' };
    }
