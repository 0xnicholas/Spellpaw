...
export async function applyStyle(params: ApplyStyleParams): Promise<ToolkitResult> {
  ...
  const sourceCard = useCanvasStore.getState().getCurrentNodes().find((n) => n.id === params.sourceCardId);
  if (!sourceCard) {
    return { success: false, message: `未找到源卡片: ${params.sourceCardId}`, retryable: false };
  }

  const sourcePrompt = (sourceCard.data.generatedPrompt as string | undefined)
  ...
  if (!stylePrompt && params.styleCardId) {
    const styleCard = useCanvasStore.getState().getCurrentNodes().find((n) => n.id === params.styleCardId);
  ...
  if (!stylePrompt) {
    return { success: false, message: '请提供 stylePrompt 或 styleCardId', retryable: false };
  }

  const linkedNodeId = sourceCard.data.linkedTreeNodeId;
  const linkedNode = linkedNodeId ? findNode(tree, linkedNodeId) ?? null : null;
  ...
  const preferredCapabilities: Capability[] = ['styleTransfer', 'image2image', 'text2image'];
  let selectedProvider: ReturnType<typeof providerRegistry.select> | null = null;
  ...
  if (!selectedProvider || 'error' in selectedProvider) {
    return { success: false, message: '没有可用的图像生成 provider', retryable: false };
  }
  ...
  if (task.status === 'failed') {
    return { success: false, message: task.error ?? '风格迁移失败', retryable: true };
  }

  const title = linkedNode ? `${linkedNode.title}（风格化）` : `${sourceCard.data.title}（风格化）`;
  ...
  return {
    ...
    cardIds: [card.id],
    ...(task.status === 'pending' || task.status === 'processing' ? { taskId: task.taskId } : {}),
  };
}
