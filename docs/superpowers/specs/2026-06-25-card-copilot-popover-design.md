...
**防御性 effect：目标卡片被外部删除时关闭 popover**

卡片可能被多种途径删除：`NodeContextMenu → delete`（CanvasPanel.tsx:203-213）、toolRouter `remove_card`、`canvasStore.removeNode` 直接调用。如果当前 popover 指向的卡片被删除，popover 仍会渲染（但指向一个不存在的 id），使用体验劣化。增加防御 effect：
...
useEffect(() => {
  ...
  if (!nodes.some((n) => n.id === copilotTarget.nodeId)) {
    setCopilotTarget(null);          // 卡片被删 → popover 关闭
  }
}, [nodes, copilotTarget]);
```

`updateNodeData` 对不存在的节点返回 state unchanged（`canvasStore.ts:156-167` 验证），所以 `useCopilotGenerate` 内部的 updateNodeData 不会崩。但 popover 会“悬空”在屏幕上指向不存在的 id，上面的体验不可接受，所以仍需这个防御 effect。
...
function useCopilotGenerate(opts: {
  ...
  const generate = useCallback(async (params: {
    ...
    try {
      ...
      // 根据是否有参考图推断 capability：有 ref → image2image / image2video，无 ref → text2image / text2video
      const capability = inferCapability(opts.kind, !!params.fileRef);
      // 参考图透传到 provider。参考 GenerationInput.referenceUrl（见 canvasToolkit/types.ts:11-19）
      // provider.submit 会在 capability 为 image2image / image2video 时实际使用 referenceUrl
...
### 4.5 kindToCardType / defaultTitle / inferCapability 辅助函数

`handlePaneCreate` 与 `useCopilotGenerate` 用到的三个映射函数：
...
// 根据 kind + 参考图是否存在推断 capability
// 有参考图 → image2image / image2video；无 → text2image / text2video
// 这是 GenerationInput.capability 的正确推断规则，详见 canvasToolkit/types.ts:5-12
// 特别提醒：doubaoProvider 在 text2image 分支会静默丢弃 referenceUrl（见 providers/doubaoProvider.ts:167-189），
// 所以必须正确选择 capability，否则用户上传的参考图会没效果。
function inferCapability(kind: CopilotKind, hasRef: boolean): Capability {
...
测试覆盖：
- `kindToCardType` 4 个分支
- `defaultTitle` 4 个分支（覆盖未来 i18n 抽取）
- `inferCapability` 4 分支 (image±ref + video±ref) + 2 个 throw case (text/upload) = 6 个 case
...
import { useViewport } from '@xyflow/react';   // React Flow 内置 hook
...
// 订阅 React Flow viewport 状态（pan/zoom/fitView 都会触发重渲染）
...
useEffect(() => {
  if (!copilotTarget) return;
  const onResize = () => {
    // 复用 recompute 逻辑（内联重复，但仅在 resize 事件中调用，不影响性能）
    const rf = reactFlowRef.current;
    if (!rf || !copilotTarget) return;
    const cardCenter = rf.flowToScreenPosition({
      x: copilotTarget.flowPosition.x + CARD_WIDTH / 2,
...
**为什么用 `useViewport` 而不是 `onMove`**：现有 `CanvasPanel.tsx:142-144` 只通过 `onMove` 跟踪 `zoom`，且每次 viewport 变化都调用 `setZoom` 会触发 React 重渲染。改用 React Flow 的 `useViewport()` hook（已在 `node_modules/@xyflow/react/dist/esm/hooks/useViewport.d.ts` 验证可用）能直接拿到 `{ x, y, zoom }` 且自动响应 viewport 变化，无需手动 setState。

**perf 考虑**：`useViewport()` 在 pan 期间会高频触发重渲染。v1 接受这个性能成本；如果用户反馈 popover 拖动卡顿，后续可改为 `useReactFlow().getViewport()` + `requestAnimationFrame` 节流。
...
5. P 输入 prompt，点 Generate
   ...
   └─ 完成: updateNodeData(id, { thumbnail: resultUrl, generatedPrompt, sourceProvider, isPlaceholder: false })
      → status → 'done'
      → setTimeout POPOVER_AUTOCLOSE_DELAY_MS (= 1200ms, 见 §4.10) 后 setCopilotTarget(null)  // 弹窗关闭
```
...
| 用户取消（AbortController） | status → 'idle'，**保留之前输入**（用户可修改后重试）。prompt 不自动清空——用户取消可能是想重新看自己之前写的提示词。 |
...
- upload kind 写入 thumbnail/title/fileSize/fileRef
- image kind + 无 ref：调用 providerRegistry.submit({ capability: 'text2image' }) + pollUntilDone
- image kind + 有 ref：调用 providerRegistry.submit({ capability: 'image2image', referenceUrl: dataUrl })
- video kind + 无 ref：capability = 'text2video'
- video kind + 有 ref：capability = 'image2video', referenceUrl 透传
- 生成成功 updateNodeData 卡片的 thumbnail/prompt/provider
- 生成失败捕获错误并 setStatus('error')
- AbortController 触发后回到 idle
- 组件卸载 abortRef.abort()

**inferCapability.test.ts**（新）：
- image 无 ref → 'text2image'
- image 有 ref → 'image2image'
- video 无 ref → 'text2video'
- video 有 ref → 'image2video'
- text → throws
- upload → throws
...
- 点击空白关闭 popover
- Esc 关闭 popover
- popover 位置跟随 viewport pan/zoom 更新（模拟 useViewport 变化 → screen position 重算）
- popover 位置在 window resize 后更新
- self-click guard：同一卡片重复点击不触发 setState
- 目标卡片被删除后 popover 自动关闭（NodeContextMenu delete / toolRouter remove_card）
- popover JSX 使用 `key={nodeId}` 验证（切换卡片强制 re-mount，触发 abortRef）
...
- 端到端: 选中 text 卡片上传 ref 图 → capability 推断为 text2image（ref 不适用）/ 选中 image 卡片上传 ref 图 → capability 推断为 image2image（ref 生效）

**canvasStoreMigration.test.ts**（新）：
- 装载 v3 持久化状态（含 5 个 copilotCard 节点，4 种 kind 混合）→ hydrate 后验证：
  - 所有 copilotCard 节点已被转换为对应正式类型
  - status='done' 的节点保留 thumbnail 和 sourceProvider
  - status='generating'/'idle' 的节点 isPlaceholder=true
  - version 提升为 4
- v4 状态再装载一次 → 不重复迁移（幂等）
...
- inferCapability: ~6 tests
- CanvasPanel 扩展: ~10 tests（含 viewport / resize / self-click / 外部删除）
- GenericCardNode 扩展: ~3 tests（**新文件**，原不存在）
- 集成测试: ~5 tests（含 capability 端到端）
- migrateCopilotCards 函数: ~14 tests（§14）
- canvasStore persist.migrate 集成: ~3 tests

目标：总计 **≥ 358 tests passing**。
...
- popover 位置在卡片拖拽后更新
- popover 位置在 window resize 后更新
- 已有 copilotCard 节点迁移（done / generating / idle 三状态，4 种 kind）
- capability 根据 ref 推断正确（image2image / image2video）
...
❌ **`CanvasPanel` useEffect**（初始设计）：useEffect 在 render + paint 之后才运行，会出现“v1 节点丢失 → 迁移后重新出现”的闪现，用户体验差。
