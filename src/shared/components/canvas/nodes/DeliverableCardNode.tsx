...
export function DeliverableCardNode({ data, id, selected }: NodeProps<Node<CanvasNodeData>>) {
  ...
  return (
    <CanvasCard
      ...
      selected={selected}
      label="产出物"
      iconOverride={Package}
      ariaLabel={`产出物：${data.title}`}
    >
      <Handle type="target" position={Position.Left} className="!bg-[var(--color-accent-500)]" />

      {/* Sub-type indicator (image/video/audio) */}
      <div className="px-3 pb-1.5 flex items-center gap-1 text-[10px] text-[var(--color-text-tertiary)]">
        <config.icon className="h-2.5 w-2.5" aria-hidden="true" />
      ...
      {deliverableType === 'audio' && (
        <AudioContent duration={duration} fileSize={fileSize} />
      )}
