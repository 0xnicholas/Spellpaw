...
|------|------|
| **Phase 1** | ✅ 完成 — 本地内容编辑工具 |
| **Phase 2** | ✅ 完成 — AI 创作助手（Pandaria 集成 · Tool Server · 模板系统 · Chat SSE · 节奏分析） |
...
> **前置条件：** 如需 AI Agent 功能，需在 `../pandaria` 启动 Pandaria 后端。仅有前端可体验完整的本地编辑功能。
...
| Agent 后端 | [Pandaria](https://github.com/anthropics/pandaria)（Rust） |
...
Spellpaw **不自己跑 LLM**。Agent 能力由 Pandaria（Rust 后端）通过 REST + SSE + HttpProxyTool 提供：

```
Spellpaw (Browser)          Pandaria (Rust Backend)
─────────────               ────────────────────────
① 启动 Tool Server  ←───→  ② 创建 Session + 注册 Tools
③ 订阅 SSE 事件  ←───→  ④ 发送消息 → LLM 推理
⑤ UI 渲染 tool 进度  ←───→  ⑥ Tool Call → HTTP → Tool Server → Store Action
```

详见 `docs/decisions/ADR-001-pandaria-agent-backend.md` 和 `docs/decisions/ADR-002-tool-server-architecture.md`。

### Tool Server：WebSocket 桥接

Pandaria 通过 HTTP 调用 tool，但 Zustand store 在浏览器内存。Vite 插件作为 HTTP ↔ WebSocket 转发层，不碰业务逻辑。
...
| `../pandaria/docs/openapi.yaml` | Pandaria API 规格 |
