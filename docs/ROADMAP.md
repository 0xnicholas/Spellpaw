...
> **路线图调整：** 竞争关键项前移、新增 Phase 2.5「竞争加固」、多语言从 Phase 4 提升至 Phase 2.5。视频/图像生成通过 Spellpaw Server + Tokencamp 网关。  
...
| 视频生成闭环 | 12+ 模型 API 聚合，直接出片 | 🔴 高 | 通过 Spellpaw Server + Tokencamp 网关补齐能力（不自研模型） |
...
1. **不自研 AI 模型** — 通过 Spellpaw Server + Tokencamp 网关补齐生成能力
...
> **架构决策：** Spellpaw 不跑 LLM。Agent 能力由 Spellpaw Server 提供——通过 REST + SSE + Tool Calling 机制集成。Spellpaw 负责：启动本地 Tool Server、注入项目上下文到 system_prompt、消费 SSE 事件流。
...
> **注：** 视频/图像生成通过 Spellpaw Server tool（`generate_storyboard`）→ Tokencamp 网关路由至模型厂商。
...
**架构决策：** Spellpaw **不跑 LLM**。Agent 能力由 Spellpaw Server 提供——一个完整的 Agent 会话后端（REST + SSE + Tool Calling 机制）。Spellpaw 的职责是三件事：
1. **启动本地 Tool Server** — 让 Spellpaw Server 通过 HTTP 调用 Spellpaw 的 store actions
2. **上下文注入** — 将项目结构写入 session 的 `system_prompt`，LLM 在对话开始即了解项目全貌
3. **SSE 消费** — 监听流式事件，渲染 Chat UI，展示 tool 执行进度

```
Spellpaw                                      Spellpaw Server
────────                                      ─────────────
...
④ 监听 SSE 流 ←────────────────────────────── Spellpaw Server 处理中
   ┌─ text_delta        → Chat 面板流式渲染
   ├─ tool_call_started  → 显示"🔧 正在操作..."
   │                                              │
   │   POST localhost:9341/tool ──────────────→ Spellpaw Server 调用 tool endpoint
   │   { tool_call_id, params }                  │
...
**Spellpaw Server 发来的请求（Tool Calling 格式）：**
...
##### Copilot Session 配置与上下文管理

**核心策略：A+B 结合——turn 间自动 PATCH system_prompt + 镜头细节按需 tool 获取。**

Spellpaw Server 支持跨 turn 更新 system_prompt（`PATCH /sessions/{id}`），所以 Spellpaw 不需要把全部数据一次性塞入。采用两层上下文：
...
function useSystemPromptSync(sessionId: string) {
  ...
  useEffect(() => {
    ...
    fetch(`${SPELLPAW_SERVER}/api/v1/sessions/${sessionId}`, {
      method: 'PATCH',
...
分步协作不靠前端卡片 UI 控制——它不需要。tool 一旦被调用，结果已经持久化到 store 了。分步的节奏由 **system_prompt 规则**驱动：
...
**风险缓解：** Topview API 可能随时变更或收费。Spellpaw 的桥接层设计为**可插拔 provider 模式**——用户也可选择其他视频生成 API（如 Runway, Pika），或通过 Spellpaw Server 的 generate_storyboard tool 生成静态分镜图（fallback）。
...
Week 3-4:   Tool Server + Agent 分步协作（提前）
            ...
            └── Week 4: Copilot session 配置 + system_prompt 注入
                        + SSE 事件消费基础通
...
| 决策点 | 选型 | 理由 |
|--------|------|------|
| Agent 后端 | Spellpaw Server / 可插拔 LLM 提供商 | REST + SSE + Tool Calling 机制 |
...
| Chat 流式渲染 | SSE `text_delta` + `tool_call_*` 事件 | Spellpaw Server 原生 SSE |
| 图像生成 | Tool endpoint 内部转发到 DALL·E / Stability API | Spellpaw Server 不负责图像 |
| 🆕 视频生成 | **Topview REST API 桥接**（可插拔 provider） | 不自研模型；免费 tier；单 API 12+ 模型 |
| 模板存储 | `.spellpaw-template.json`（本地文件） | 与项目 JSON 格式统一 |

### 2.5 Tool Server 架构

**核心矛盾：** Spellpaw Server 通过 HTTP POST 调用 tool，但 Zustand store 在浏览器内存。HTTP server 无法直接访问 store。

**方案：** Vite 插件内嵌 HTTP server + WebSocket bridge。Tool Server 作为 HTTP ↔ WebSocket 的消息转发层，不碰业务逻辑。

```
Spellpaw Server               Vite Dev Server (:5173)            Browser
───────────────              ─────────────────────────           ───────
...
function spellpawToolServer(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      // HTTP: Spellpaw Server 的 tool call 入口
...
| Tool Server 未启动 | Spellpaw Server Tool Calling 连接拒绝 → SSE `error` 事件 → Chat 面板提示 |
...
Spellpaw Server 的 tool endpoint 指向 `http://localhost:PORT/tool`（Electron main process 上的 HTTP server），其余不变。

### 2.7 开发与测试策略

Phase 2 各模块必须能独立开发，不等后端就绪。依赖关系决定开发顺序：
...
Tool Server ─────────────────── 无外部依赖，用 curl 自测
    ...
    │
    └── Copilot 对接 ────────── 最后一块，可用本地 Spellpaw Server 实例

Chat UI ────────────────────── mock SSE 先行，后端就绪后切真实流
```
...
Vite 插件启动后，在浏览器打开并连接 `/tool-ws` 的前提下，用 curl 模拟 Spellpaw Server：
...
后端就绪前，用事件序列模拟 SSE 流，开发和调试 `text_delta` 打字动画、`tool_call_*` 进度指示、`error` 处理：
...
Spellpaw Server 就绪后，跑完整链路验证：

```typescript
// e2e/copilot-integration.test.ts
// 前提：Spellpaw Server :8080，Spellpaw :5173
...
| 🆕 Topview API 不可用或收费变更 | Phase 2 | 🟡 中 | 可插拔 provider 模式；fallback 到 Spellpaw Server 静态分镜图生成 |
