  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="w-full max-w-sm rounded-[var(--radius-base)] border border-[var(--color-border-default)] bg-[var(--color-bg-primary)] p-6 shadow-lg">
        <h3 className="mb-4 text-sm font-semibold text-[var(--color-text-primary)]">Project Settings</h3>
        <div className="space-y-3">
          ...
          <div>
            <label className="mb-1 block text-[11px] font-medium text-[var(--color-text-secondary)]">Description</label>
            <Input
              value={description}
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button size="sm" disabled={!title.trim()} onClick={handleSubmit}>Save</Button>
        </div>
      </div>
    </div>
  );
