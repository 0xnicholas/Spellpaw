...
| **Phase 2** | ✅ 完成 — AI 创作助手（Pandaria 集成 · Tool Server · 模板系统 · Chat SSE · 节奏分析 · 画布缩略图） |
| **Phase 3** | 🔄 进行中 — 云端基础设施已落地（IndexedDB 缓存 · 自动同步引擎 · 冲突检测 UI · 模板市场本地版） |
| **分支** | `main` |
| **测试** | 155 tests passing / 18 files / 0 failures |
...
│   ├── customTemplateStore.ts# 用户自定义模板（localStorage 持久化）
│   ├── toolRouter.ts         # Pandaria tool → store action 路由
│   └── sync.ts               # 树 ↔ 画布双向同步
├── components/
│   ├── tree-view/            # 左栏：树状项目结构
│   ├── asset-manager/        # 左栏下区：资产管理
│   ├── chat-panel/           # 中栏：Agent 对话
│   ├── detail-panel/         # 中栏：节点详情编辑 + 节奏分析面板
│   ├── flow-canvas/          # 右栏：无限画布（含 SceneCardNode / AssetCardNode）
│   ├── template-browser/     # 模板浏览器（内置/社区/自定义）
│   ├── modals/               # 模态框（新建项目、删除确认、冲突解决等）
│   ├── sync/                 # 同步状态指示器
│   └── ui/                   # 通用 UI 组件（TabBar, FormField, EditableTitle, Lightbox 等）
├── pages/
│   ├── LoginPage.tsx         # /login
│   ├── ProjectListPage.tsx   # /projects
│   ├── TemplateMarketPage.tsx# /templates — 模板市场（本地管理）
│   └── WorkspacePage.tsx     # /project/:id — 三栏布局主界面
├── types/                    # TypeScript 类型定义（含 NarrativeTemplate）
├── hooks/                    # 自定义 hooks
├── lib/                      # 工具函数
│   ├── treeUtils.ts          # 树遍历/搜索/过滤
│   ├── treeDiff.ts           # 树 diff + 冲突合并
│   ├── projectAnalysis.ts    # 节奏分析（CV / 时长分布预警）
│   ├── templateExportImport.ts# 项目 ↔ 模板转换 + 文件 I/O
│   ├── idbStorage.ts         # IndexedDB 缓存层
│   ├── syncEngine.ts         # 自动同步引擎
│   ├── projectSync.ts        # 云端 push/pull + 冲突检测
│   └── utils.ts              # 通用工具
└── test/                     # 测试工具
```
...
CanvasNode {
  ...
  position, data: { title, description, status, linkedTreeNodeId?, thumbnail?, ... }
}
...
| `generate_storyboard` | 转发图像 API → 存入资产库 + 写入 `generatedPrompt` |
...
npm run dev:server    # 启动后端 dev server（:3001）
...
✅ 阶段 3: Pandaria 对接（session 配置 + SSE 消费）
  → src/hooks/usePandariaSSE.ts + src/lib/pandaria.ts
  → ChatPanel 已切换为真实 SSE

✅ 阶段 4: 叙事模板系统
  → src/types/index.ts (NarrativeTemplate)
  → src/components/template-browser/TemplateBrowser.tsx
  → public/templates/*.spellpaw-template.json (5 个内置模板)

✅ 阶段 5: 模板系统完善（自定义模板导出/导入 + 模板市场本地版）
  → src/stores/customTemplateStore.ts
  → src/lib/templateExportImport.ts
  → src/pages/TemplateMarketPage.tsx

✅ 阶段 6: AI 分镜画布体验（缩略图 lightbox + 批量生成 + 风格锁）
  → src/components/ui/Lightbox.tsx
  → src/components/flow-canvas/nodes/SceneCardNode.tsx（thumbnail 展示）

✅ 阶段 7: 智能分析与推荐（节奏分析）
  → src/lib/projectAnalysis.ts + projectAnalysis.test.ts
  → src/components/detail-panel/AnalysisReport.tsx
```
...
- 工具函数：`src/lib/{name}.ts`，测试 `{name}.test.ts`
- Phase 2 新增：`src/stores/toolRouter.ts`、`tool-server/`
- Phase 3 新增：`src/lib/idbStorage.ts`、`src/lib/syncEngine.ts`、`src/lib/migrateToIDB.ts`、`src/lib/treeDiff.ts`、`src/lib/projectAnalysis.ts`、`src/lib/templateExportImport.ts`、`src/stores/customTemplateStore.ts`
...
- 开发期：Pandaria 可本地启动（`../pandaria`），工具调用通过 `http://127.0.0.1.nip.io:5173/tool`（使用 `nip.io` 域名绕过 Pandaria SSRF 防护，实际解析到 127.0.0.1）
- Pandaria 的完整 API 规格见 `../pandaria/docs/openapi.yaml`

---

*最后更新：2026-05-30（模板系统完善 · 画布缩略图 lightbox · 节奏分析 · 冲突检测 UI · 模板市场本地版）*
