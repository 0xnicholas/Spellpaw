...
| **Phase 2** | ✅ 基本完成 — AI 创作助手（Pandaria 集成 · Tool Server · 模板系统 · Chat SSE） |
| **Phase 3** | 🔄 进行中 — 云端基础设施已落地（IndexedDB 缓存 · 自动同步引擎 · 冲突检测 UI） |
| **分支** | `main` |
| **测试** | 58 tests passing / 8 files / 0 failures |
...
├── hooks/               # 自定义 hooks
├── lib/                 # 工具函数（treeUtils, utils, idbStorage, syncEngine, projectSync）
└── test/                # 测试工具
```
...
- Phase 3 新增：`src/lib/idbStorage.ts`、`src/lib/syncEngine.ts`、`src/lib/migrateToIDB.ts`
...
*最后更新：2026-05-29（IndexedDB 缓存层 + 自动同步引擎上线）*
